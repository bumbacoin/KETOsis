...
#if defined(NDEBUG)
# error "ketosis cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ketosis Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    if(pindexBest->nHeight == 1) {
        nSubsidy = 1000000 * COIN; //1 Million Premine
	}
...
int64_t nTargetTimespan = 12 * 60 * 60;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    else if (pindexLast->nHeight < 10080)
